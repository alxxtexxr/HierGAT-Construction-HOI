...
def main(
    ...
    batch_size, val_fraction = (
        cfg.models.optimization.batch_size,
        cfg.models.optimization.val_fraction,
    )
    ...
    )  # Count occurrences of each feature directory
    # and sort from lowest to highest
    # kfold_test_feature_dirs = feature_dir_counts.index[:k] # OLD
    
    # NEW - Monte Carlo mechanism: Randomly sample k feature directories as test feature directories
    kfold_test_feature_dirs = random.sample(feature_dir_counts.index.tolist(), k)
    ...
    for fold, test_feature_dir in enumerate(kfold_test_feature_dirs):
        ...
        all_video_ids = [
            feature_dir.parent.stem.split("_")[0] for feature_dir in FEATURE_DIRS
        ]
        # train_video_ids = [feature_dir.parent.stem.split('_')[0] for feature_dir in FEATURE_DIRS if feature_dir != test_feature_dir]
        train_video_ids = [
            video_id for video_id in all_video_ids if video_id != test_video_id
        ]
        ...
        test_feature_dirs_df = feature_dirs_df[
            feature_dirs_df["base_dir"] == test_feature_dir
        ]
        train_feature_dirs_df = feature_dirs_df[
            feature_dirs_df["base_dir"] != test_feature_dir
        ]
        ...
        for base_dir, group_df in df.groupby("base_dir"):
            ...
            assert duration // 2 >= val_duration + val_duration, (
                f"Duration ({duration}) should be at least twice the validation duration ({val_duration})!"
            )
            # val_start_fid = random.randrange(0, duration//2) # Sample a random FID from the start to the middle
            val_start_fid = random.randrange(
                duration // 2, duration - val_duration - val_duration
            )  # Sample a random FID from the middle to the end
            ...
            print(
                f"Validation FID range: {val_start_fid}-{val_end_fid}, length: {val_end_fid - val_start_fid + 1}"
            )
            print(
                f"Testing FID range: {test_start_fid}-{test_end_fid}, length: {test_end_fid - test_start_fid + 1}"
            )
            print()
            ...
            val_len = len(
                df.loc[
                    base_dir_cond
                    & (df["start_fid"] >= val_start_fid)
                    & (df["end_fid"] <= val_end_fid)
                ]
            )
            test_len = len(
                df.loc[
                    base_dir_cond
                    & (df["start_fid"] >= test_start_fid)
                    & (df["end_fid"] <= test_end_fid)
                ]
            )
            if val_len > test_len:
            ...
            try:
                assert (
                    len(test_group_df) > len(val_group_df) or len(test_group_df) == 0
        ...
        device = (
            "cuda" if torch.cuda.is_available() and cfg.resources.use_gpu else "cpu"
        )
        model = TGGCN(feat_dim=1024, **model_creation_args).to(device)
        if (
            misc_dict.get("pretrained", False)
            and misc_dict.get("pretrained_path") is not None
        ):
        ...
        criterion, loss_names = select_loss(
            model_name, model_input_type, dataset_name, cfg=cfg
        )
        mtll_model = None
        if misc_dict.get("multi_task_loss_learner", False):
            ...
            mtll_model = MultiTaskLossLearner(loss_types=loss_types, mask=mask).to(
                device
            )
        ...
        checkpoint = train(
            ...
            feed_model_data=feed_model_data,
