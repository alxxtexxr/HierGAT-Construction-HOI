...
from src.utils import printh, get_feature_dirs_df
...
def main(
    ...
    for i, dataset in enumerate(datasets, 1):
        ...
        print(f"\n{'='*64}")
        print(f"[{i}/{len(datasets)}] Processing dataset: {video_id}")
        print(f"{'='*64}\n")
        ...
        if len(test_feature_dirs_df) == 0:
            print(f"  No valid feature dirs, skipping")
            datasets_skipped += 1
            continue

        print(f"  Feature dirs: {len(test_feature_dirs_df)}")
        ...
        print(f"  Samples: {len(test_data[0])}")

        if model is None:
            input_size = input_size_from_data_loader(
                test_loader, model_name, model_input_type
            )
    ...
    print(f"{'='*64}")
    print(f"  Total datasets: {len(datasets)}")
    print(f"  Processed: {len(datasets_processed)}")
    print(f"  Skipped: {datasets_skipped}")
    print(f"  Errors: {datasets_errors}")
    print(f"{'='*64}\n")

    if len(datasets_processed) == 0:
        print("No datasets processed. Exiting.")
        return

    printh("Computing Aggregated Metrics", 96)

    combined_output = torch.cat(all_outputs, dim=0)
    combined_target = torch.cat(all_targets, dim=0)

    y_pred = torch.argmax(combined_output, dim=1).cpu().numpy()
    y_true = combined_target.squeeze(-1).mode(dim=1).values.cpu().numpy()
    ...
    print("=" * 64)
    print(f"Total samples (all datasets): {total_samples}")
    ...
    ax.set_title(
        f"Aggregated Confusion Matrix - Fold {fold_number} ({total_samples} samples)",
        fontsize=14,
        pad=10,
    )
