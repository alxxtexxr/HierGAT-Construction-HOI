...
class TGGCN(nn.Module):
    def __init__(self, input_size: tuple, num_classes: tuple, feat_dim: int = 2048, hidden_size: int = 128,
        ...
        # self.geometry_bd_rnn = nn.GRU(hidden_size, hidden_size, num_layers=1, bias=bias, batch_first=True,
        #                               bidirectional=False)
        # self.geometry_bd_embedding_mlp = build_mlp([hidden_size, hidden_size], ['relu'], bias=bias)
        ...
        self.human_bd_rnn = nn.GRU(hidden_size, hidden_size, num_layers=1, bias=bias, batch_first=True,
                                   bidirectional=False)
        self.human_bd_embedding_mlp = build_mlp([hidden_size, hidden_size], ['relu'], bias=bias)
        ...
        # self.object_bd_rnn = nn.GRU(hidden_size, hidden_size, num_layers=1, bias=bias, batch_first=True,
        #                             bidirectional=False)
        # self.object_bd_embedding_mlp = build_mlp([hidden_size, hidden_size], ['relu'], bias=bias)
        ...
        if share_level_mlps and not cat_level_states:
            ...
            # CUSTOM
            self.human_frame_recognition_mlp = build_mlp([hidden_size, num_subactivities],
                                                         [{'name': 'logsoftmax', 'dim': -1}], bias=bias)
            self.human_frame_prediction_mlp = build_mlp([hidden_size, num_subactivities],
                                                        [{'name': 'logsoftmax', 'dim': -1}], bias=bias)
    ...
    def forward(self, x_human, x_objects, objects_mask, human_segmentation=None, objects_segmentation=None,
        ...
        # CUSTOM: Apply Global Average Pooling to hidden outputs before passing to the final MLP layers
        h_hfr = h_hfr.mean(dim=1).squeeze(1) # [batch_size, hidden_dim]
        hx_hs = hx_hs.mean(dim=1).squeeze(1) # [batch_size, hidden_dim]
        
        y_human_frame_recognition = self.human_frame_recognition_mlp(h_hfr).contiguous() # [batch_size, n_class]
        y_human_frame_prediction  = self.human_frame_prediction_mlp(h_hfr).contiguous()  # [batch_size, n_class]

        y_human_recognition = self.human_recognition_mlp(hx_hs).contiguous() # [batch_size, n_class]
        y_human_prediction  = self.human_prediction_mlp(hx_hs).contiguous()  # [batch_size, n_class]
