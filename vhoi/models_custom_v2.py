...
class TGGCN(nn.Module):
    def __init__(self, seq_len: int, input_size: tuple, num_classes: tuple, hidden_size: int = 128,
        ...
        # geometry
        self.geometry_embedding_gcn = Geo_gcn(self.seq_len, self.gcn_node, 4, 128)
    ...
    def forward(self, x_human, x_objects, objects_mask, human_segmentation=None, objects_segmentation=None,
        ...
        if self.message_geometry_to_objects:
            # [OLD] x_geometry -> [batch_size, num_frames, num_humans, feat_dim]
            # [NEW] x_geometry -> [batch_size, num_frames, num_humans, seq_len, feat_dim]
            x_geometry = x_geometry[:, :, 0, :, :] # [batch_size, num_frames, seq_len, feat_dim]
            bs, t, seq_len, vw = x_geometry.size()
            x_geometry = x_geometry.reshape(bs, t, seq_len, vw // 4, 4) # [batch_size, num_frames, seq_len, feat_dim/4, 4]
            x_geometry = x_geometry.permute(0, 4, 3, 2, 1).contiguous() # [batch_size, 4, feat_dim/4, seq_len, num_frames]
            x_geometry = self.geometry_embedding_gcn(x_geometry)
            print("x_geometry.shape:", x_geometry.shape)
            x_geometry = x_geometry.unsqueeze(2)
            print("unsqueeze/x_geometry.shape:", x_geometry.shape)
            x_geometry = x_geometry.view(bs, t, 1, seq_len, x_geometry.shape[1] * x_geometry.shape[3])
        ...
        temporal_x = x.reshape(x.shape[3], -1, x.shape[4])
        print("temporal/temporal_x.shape:", temporal_x.shape)
        ...
        # spatial information
        print("spatial/x.shape:", x.shape)
        spatial_x = x.reshape(-1, x.shape[2], x.shape[4]).permute(1, 0, 2)
        print("spatial/spatial_x.shape:", spatial_x.shape)
        spatial_x = self.spatial_self_attn(spatial_x).permute(1, 0, 2)
        print("spatial/spatial_x.shape:", spatial_x.shape)
        spatial_x = spatial_x.reshape(
            batch_size, 
            -1, 
            x_shape_2,
            self.seq_len,
        ...
        print("x_human_temporal.shape:", x_human_temporal.shape)
        print("x_human_spatial.shape:", x_human_spatial.shape)
            ...
            for h in range(num_humans):
                ...
                for s in range(self.seq_len):                
                    if self.message_humans_to_human:
                        try:
                            hh_dists = human_human_distances[:, t, s]
                        except TypeError:
                            hh_dists = None
    ...
    def _process_frame_level_rnn(x, rnn, embedding_mlp):
        ...
        Arguments:
            x - Tensor of shape (batch_size, num_steps, num_entities, seq_len, hidden_size) containing the frame-level input
        ...
        """
        batch_size, num_steps, num_entities, seq_len, hidden_size = x.shape
        h_f = []

        for e in range(num_entities):
            x_e = x[:, :, e]  # [batch_size, num_steps, seq_len, hidden_size]
            x_e = x_e.reshape(batch_size * num_steps, seq_len, hidden_size)
            h_fe, _ = rnn(x_e)  # [batch_size*num_steps, seq_len, rnn_hidden_size]

            # reshape back, infer hidden dim in case RNN is bidirectional
            h_fe = h_fe.view(batch_size, num_steps, seq_len, -1)

            h_f.append(h_fe)

        # stack along entities
        h_fr = torch.stack(h_f, dim=2)  # [batch, num_steps, num_entities, seq_len, hidden_dim]
    ...
    def _objects_to_human_message(self, x_hfth, x_oft, h_hfth, h_oft, objects_mask, ho_dists=None):
        ...
        """
        receiver = torch.cat([x_hfth, h_hfth], dim=-1)
        # receiver = receiver.reshape(receiver.shape[0], -1)
        senders = torch.cat([x_oft, h_oft], dim=-1)
        # senders = senders.reshape(senders.shape[0], -1)
        ...
        if self.message_type in {'v1', 'relational'}:
            ...
            print("senders_mask.shape:", senders_mask.shape)
